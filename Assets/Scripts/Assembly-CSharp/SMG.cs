...
public class SMG : Weapon
{
	...
	public override void Init(Player player)
	{
		...
		playSoundTimer.SetTimer(0.08f, true);
		StopFire();
		elementDotTriggerBase = ElementWeaponConfig.ElementDotTriggerBase[5];
		elementDotTriggerTimes = ElementWeaponConfig.ElementDotTriggerTime[5];
		elementDotTriggerScale = ElementWeaponConfig.ElementDotTriggerScale[5];
	}
	...
	public override void PlaySound()
	{
		if (playSoundTimer.Ready())
		{
			if (!PlayElementSound())
			{
				AudioManager.GetInstance().PlaySoundAt("RPG_Audio/Weapon/smg/SMG01_fire", player.GetTransform().position);
			}
			playSoundTimer.Do();
		}
	}
	...
	public override void Reload()
	{
		...
		AudioManager.GetInstance().PlaySoundAt("RPG_Audio/Weapon/smg/SMG01_reload", player.GetTransform().position);
		elementDotTriggerTimes = ElementWeaponConfig.ElementDotTriggerTime[5];
	}
}
