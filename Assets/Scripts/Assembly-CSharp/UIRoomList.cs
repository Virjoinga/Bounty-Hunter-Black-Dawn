...
public class UIRoomList : MonoBehaviour, InGameMenuListener, UIMsgListener
{
	...
	public void LoginSuccess()
	{
		if (UITeam.giftDaily)
		{
			UITeam.m_instance.m_rewards.SetActive(true);
		}
		SendUpdateRoomsRequest(true);
	}
	...
	{
		UITeam.m_instance.SetMenuClose(false);
		UILoadingNet.m_instance.Hide();
	}

	public void JoinRoomFailed()
	{
		if (UITeam.m_instance.m_password.activeSelf)
		{
			UITeam.m_instance.m_password.SetActive(false);
		}
		UILoadingNet.m_instance.Hide();
	...
	public void SendUpdateRoomsRequest(bool immediate)
	{
		...
		if ((DateTime.Now - m_start).TotalSeconds >= 60.0)
		{
			...
			if (string.IsNullOrEmpty(text))
			{
				...
				GetRoomListRequest request = new GetRoomListRequest(0, charLevel, maxUnlockCity, currentQuest);
				GameApp.GetInstance().GetNetworkManager().SendRequest(request);
			}
			else
			{
				Debug.Log("lowerText:" + text);
				SearchRoomRequest request2 = new SearchRoomRequest(Mode.CamPain, text);
	...
	{
		UITeam.m_instance.m_roomList.gameObject.SetActive(false);
		UITeam.m_instance.m_createRoom.SetActive(true);
	}
	...
	public Room GetCurRoom()
	{
		...
		{
			if (room.getRoomID() == UITeam.m_instance.m_roomList.m_currRoomId)
			{
				return room;
			}
		}
	...
	public void OnClickJoin(GameObject go)
	{
		...
		if (curRoom != null)
		{
			...
			if (curRoom.isHasPassword())
			{
				...
				UITeam.m_instance.m_password.SetActive(true);
				return;
			}
