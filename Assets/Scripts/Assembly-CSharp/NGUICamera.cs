...
public class NGUICamera : MonoBehaviour
{
	...
	public static NGUICamera current = null;
	...
	private static List<NGUICamera> mList = new List<NGUICamera>();
	...
	public static GameObject selectedObject
	{
		...
		set
		{
			...
			if (mSel != null)
			{
				NGUICamera nGUICamera = FindCameraForLayer(mSel.layer);
				if (nGUICamera != null)
				{
					current = nGUICamera;
					currentCamera = nGUICamera.mCam;
					Notify(mSel, "OnSelect", false);
					if (nGUICamera.useController || nGUICamera.useKeyboard)
					{
			...
			NGUICamera nGUICamera2 = FindCameraForLayer(mSel.layer);
			if (nGUICamera2 != null)
			{
				current = nGUICamera2;
				currentCamera = nGUICamera2.mCam;
				if (nGUICamera2.useController || nGUICamera2.useKeyboard)
				{
					Highlight(mSel, true);
				}
	...
	public static Camera mainCamera
	{
		get
		{
			NGUICamera nGUICamera = eventHandler;
			return (!(nGUICamera != null)) ? null : nGUICamera.cachedCamera;
		}
	}

	public static NGUICamera eventHandler
	{
		get
		{
			for (int i = 0; i < mList.Count; i++)
			{
				NGUICamera nGUICamera = mList[i];
				if (!(nGUICamera == null) && nGUICamera.enabled && NGUITools.GetActive(nGUICamera.gameObject))
				{
					return nGUICamera;
				}
			}
	...
	private static int CompareFunc(NGUICamera a, NGUICamera b)
	{
	...
	private static bool Raycast(Vector3 inPos, ref RaycastHit hit)
	{
		...
		{
			NGUICamera nGUICamera = mList[i];
			if (!nGUICamera.enabled || !NGUITools.GetActive(nGUICamera.gameObject))
			{
				continue;
			}
			currentCamera = nGUICamera.cachedCamera;
			...
			int layerMask = currentCamera.cullingMask & (int)nGUICamera.eventReceiverMask;
			float distance = ((!(nGUICamera.rangeDistance > 0f)) ? (currentCamera.farClipPlane - currentCamera.nearClipPlane) : nGUICamera.rangeDistance);
			if (nGUICamera.clipRaycasts)
			{
	...
	public static NGUICamera FindCameraForLayer(int layer)
	{
		int num = 1 << layer;
		for (int i = 0; i < mList.Count; i++)
		{
			NGUICamera nGUICamera = mList[i];
			Camera camera = nGUICamera.cachedCamera;
	...
	private void ProcessMouse()
	{
		...
		if (flag)
		{
			...
			if (NGUICursor.IsDragging())
			{
				mousePosition.y += 50f;
			}
			GameObject gameObject = ((!Raycast(mousePosition, ref lastHit)) ? fallThrough : lastHit.collider.gameObject);
			if (gameObject == null)
			{
				gameObject = genericEventHandler;
			}
	...
	private void ProcessTouch(bool pressed, bool unpressed)
	{
		...
		if (currentTouch.pressed != null)
		{
			...
			if (currentTouch.pressed == currentTouch.current || (currentTouch.clickNotification != 0 && currentTouch.totalDelta.magnitude < num))
			{
				...
				{
					mSel = currentTouch.pressed;
					Notify(currentTouch.pressed, "OnDropFail", true);
				}
			...
			{
				mSel = currentTouch.current;
				Notify(currentTouch.current, "OnDrop", currentTouch.pressed);
			}
			else
			{
				Notify(currentTouch.pressed, "OnDropFail", currentTouch.pressed);
			}
		}
		currentTouch.dragStarted = false;
