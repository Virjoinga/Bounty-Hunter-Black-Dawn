...
public class RobotConnection
{
	...
	public void SendData(Request request)
	{
		...
		catch (Exception ex)
		{
			...
			if (networkMgr != null)
			{
				networkMgr.CloseRobotConnection();
			}
		}
	}
		...
		try
		{
			...
			while (true)
			{
				lock (this)
				{
					...
					{
						object obj = null;
						byte[] array = new byte[session.DatagramLength - 4 - message.Content.Length];
						Array.Copy(session.DatagramBuffer, message.Content.Length + 4, array, 0, array.Length);
	...
	public void StartConn(string ip, int port)
	{
		...
		try
		{
			...
			Debug.Log("starting connection..." + ip + "," + port);
