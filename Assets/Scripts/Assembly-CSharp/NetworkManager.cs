...
public class NetworkManager
{
	...
	public void StartNetworkRobot(RobotUser robot, string ip, int port)
	{
		receivedPacketCache = new ReceivedPacketCache();
		sendPacketCache = new SendPacketCache();
	...
	{
		if (GameApp.GetInstance().GetGameMode().PlayerStatus != 0 || ((Request)request).requestID <= 100)
		{
	...
	public void SendRequestAsRobot(object request, RobotUser robot)
	{
		if (robot.GetGameApp().GetGameMode().PlayerStatus != 0 || ((Request)request).requestID <= 100)
		{
			sendPacketCache.AddPacket(request);
			updatePacket(request);
		}
	...
	{
		if (robotConn != null && robotConn.IsConnected && !getSendPacketCache().isEmpty() && !sending)
		{
	...
	{
		packetNum++;
		byteNum = byteNum + 2 + ((Request)request).length;
		...
		if ((float)timeSpan.TotalSeconds > 1f)
		{
			...
			{
				Debug.LogWarning("Too many packets in one seconds:" + GameApp.GetInstance().AverageSendPacket);
			}
			packetNum = 0;
			byteNum = 0;
	...
	public void ProcessRobotReceivedPackets(RobotUser robot)
	{
		while (!receivedPacketCache.isEmpty())
		{
			Response response = (Response)receivedPacketCache.RetrivePacket();
