...
public class LaserRifle : Weapon
{
	...
	public override void Attack(float deltaTime)
	{
		...
		for (int j = 0; j < array.Length; j++)
		{
			...
			if (GameApp.GetInstance().GetGameMode().IsMultiPlayer() && GameApp.GetInstance().GetGameMode().IsVSMode() && GameApp.GetInstance().GetGameMode().IsTeamMode() && array[j].collider.gameObject.layer == PhysicsLayer.REMOTE_PLAYER)
			{
