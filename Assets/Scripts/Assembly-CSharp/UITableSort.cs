...
[ExecuteInEditMode]
[AddComponentMenu("NGUI/Interaction/TableSort")]
public class UITableSort : MonoBehaviour
{
	...
	public List<Transform> children
	{
		get
		{
			if (mChildren.Count == 0)
			{
				...
				if (sorted)
				{
					if (sortRule == SortRule.Default)
					{
						mChildren.Sort(SortByName);
					}
					else if (sortRule == SortRule.Numeric)
					{
						mChildren.Sort(SortByNum);
					}
				}
			}
			return mChildren;
	...
	public static int SortByNum(Transform a, Transform b)
	{
		if (Convert.ToInt32(a.name) < Convert.ToInt32(b.name))
		{
			return -1;
		}
		return 1;
	...
	private void Start()
	{
		...
		if (keepWithinPanel)
		{
			...
			mDrag = NGUITools.FindInParents<UIDraggablePanelAlign>(base.gameObject);
		}
		Reposition();
	}
