...
public class Pistol : Weapon
{
	...
	{
		return 2.17f * base.ReloadTimeScale / reloadTime;
	}
	...
	public override void Init(Player player)
	{
		...
		playSoundTimer.SetTimer(0.2f, true);
		StopFire();
		elementDotTriggerBase = ElementWeaponConfig.ElementDotTriggerBase[2];
		elementDotTriggerTimes = ElementWeaponConfig.ElementDotTriggerTime[2];
		elementDotTriggerScale = ElementWeaponConfig.ElementDotTriggerScale[2];
	}
	...
	public override void PlaySound()
	{
		if (playSoundTimer.Ready())
		{
			if (!PlayElementSound())
			{
				AudioManager.GetInstance().PlaySoundAt("RPG_Audio/Weapon/pistol/pistol_fire", player.GetTransform().position);
			}
			playSoundTimer.Do();
		}
	}
	...
	public override void Reload()
	{
		...
		AudioManager.GetInstance().PlaySoundAt("RPG_Audio/Weapon/pistol/pistol_reload", player.GetTransform().position);
		elementDotTriggerTimes = ElementWeaponConfig.ElementDotTriggerTime[2];
	}
}
