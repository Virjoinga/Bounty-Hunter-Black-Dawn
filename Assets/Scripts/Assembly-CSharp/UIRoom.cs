...
public class UIRoom : MonoBehaviour
{
	...
	private void OnClick()
	{
		...
		if (!isPlayingAnimation)
		{
			...
			{
				UITeam.m_instance.m_roomList.SendGetRoomData(m_id);
				UITeam.m_instance.m_roomList.Selection(m_id);
			}
			else
			{
				UITeam.m_instance.m_roomList.m_currRoomId = -1;
			}
			NGUITools.SetActive(UITeam.m_instance.m_roomList.m_join, false);
		}
	}
