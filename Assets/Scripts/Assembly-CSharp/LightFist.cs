...
public class LightFist : Weapon
{
	...
	public override void Loop(float deltaTime)
	{
		...
		if (!attacked && (player.AnimationPlayed(attack + GetAnimationSuffix(), 0.5f) || player.AnimationPlayed(AnimationString.RunAttack + GetAnimationSuffix(), 0.5f)))
		{
			...
			GameObject original = Resources.Load("Effect/Fist_Shot") as GameObject;
			...
			{
				PlayerFireRocketRequest request = new PlayerFireRocketRequest(15, gunfire.position, normalized2, ElementType.NoElement);
